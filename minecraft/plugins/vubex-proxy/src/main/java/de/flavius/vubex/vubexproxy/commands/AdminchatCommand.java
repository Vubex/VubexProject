...
public class AdminchatCommand extends Command {
    ...
    public AdminchatCommand(VubexDiscordAPI vubexDiscordAPI) {
        super("adminchat", null, "ac");
        this.vubexDiscordAPI = vubexDiscordAPI;
    }

    static String adminChatPermission = "vubex.adminchat";

    @Override
    public void execute(CommandSender sender, String[] args) {
        if(!sender.hasPermission(adminChatPermission)){
        ...
        if (args.length < 1) {
            ...
            sender.sendMessage(new TextComponent(Vubex_proxy.serverPrefix + ChatUtils.getColoredText("&7Verwendung: &e/adminchat [Nachricht]")));
            return;
        }
        ...
        for (ProxiedPlayer player : ProxyServer.getInstance().getPlayers()) {
            if (player.hasPermission(adminChatPermission)) {
                player.sendMessage(new TextComponent(formattedMessage));
            }
        }
        vubexDiscordAPI.sendMessageToChannel("1222293081710399508", ChatUtils.removeFormattingCharacters(ChatUtils.getColoredText("**" + sender.getName() + "**" + ": " + message)));
        ProxyServer.getInstance().getConsole().sendMessage(new TextComponent((ChatUtils.getColoredText("&c&lADMINCHAT&r &8» &7" + sender.getName() + "&7: &7" + message))));
    }
    ...
    private String formatMessage(String senderName, String[] args) {
        ...
        return ChatUtils.getColoredText("&c&lADMINCHAT&r &8» &7" + senderName + ChatUtils.getColoredText("&7: &7") + formattedMessage.toString().trim());
    }
    ...
    public static class DiscordMessageListener extends ListenerAdapter {
        ...
        public void onMessageReceived(MessageReceivedEvent event) {
            ...
            if ((!event.getAuthor().isBot()) || textChannel.getId().equals("1222293081710399508")) {
                ...
                proxyServer.getConsole().sendMessage(new TextComponent(ChatUtils.getColoredText("&c&lADMINCHAT&r &8» &9[DISCORD] &7" + senderName + "&7: &7" + Arrays.toString(message))));
            }
        }
        ...
        private String formatMessage(String senderName, String[] message) {
            ...
            return ChatUtils.getColoredText("&c&lADMINCHAT&r &8» &9[DISCORD] &7" + senderName + ChatUtils.getColoredText("&7: &7") + formattedMessage.toString().trim());
        }
    }
}